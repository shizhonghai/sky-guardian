...
import { ArrowLeft, PlayCircle, Image, Calendar, Camera, Clock, Filter, Download, Trash2, Film, Play, Pause, SkipForward, SkipBack, AlertTriangle, RotateCcw, RotateCw, ChevronDown } from 'lucide-react';
...
const MediaLibrary: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-transparent pb-safe relative">
            {/* Header */}
            <div className="p-4 bg-blue-950/60 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between border-b border-white/10 shadow-sm">
            ...
            {activeTab === 'PLAYBACK' ? (
                <div className="flex-1 flex flex-col overflow-hidden">
                    {/* Player Area */}
                    <div className="aspect-video bg-black relative flex-shrink-0 group shadow-2xl z-10">
                        {currentSegment ? (
                            <video
                                ref={videoRef}
                        ...
                        {currentSegment && (
                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4 flex flex-col gap-2 z-20 transition-opacity duration-300">
                                {/* Progress Bar Row */}
                    ...
                    {/* Controls Bar - Increased Z-Index and Solid Background to fix visual overlap issues */}
                    <div className="p-4 bg-slate-950 border-b border-white/10 flex gap-4 relative z-20 shadow-md">
                        {/* Camera Select (Custom Dropdown) */}
                        <div className="flex-1 relative">
                            <Camera className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none z-10" size={16} />

                            <button
                                onClick={() => setIsCameraDropdownOpen(!isCameraDropdownOpen)}
                                className="w-full bg-slate-800 text-white pl-9 pr-8 py-2.5 rounded-lg border border-white/10 text-sm outline-none focus:border-blue-500 shadow-sm text-left flex items-center active:bg-slate-700/80 transition-colors"
                            >
                                <span className="truncate">{cameras.find(c => c.id === selectedCameraId)?.name || '选择监控点'}</span>
                            </button>

                            <ChevronDown
                                className={`absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none z-10 transition-transform duration-200 ${isCameraDropdownOpen ? 'rotate-180' : ''}`}
                                size={16}
                            />
                            ...
                            {isCameraDropdownOpen && (
                                <>
                                    <div className="fixed inset-0 z-30" onClick={() => setIsCameraDropdownOpen(false)}></div>
                                    <div className="absolute top-full left-0 w-full mt-2 bg-slate-900 border border-white/10 rounded-xl shadow-2xl z-40 max-h-64 overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
                                        {cameras.map(c => (
                                            <div
                                                key={c.id}
                                                onClick={() => {
                                                ...
                                                className={`px-4 py-3 text-sm cursor-pointer border-b border-white/5 last:border-0 hover:bg-slate-800/80 transition-colors ${selectedCameraId === c.id ? 'text-blue-400 bg-slate-800/40' : 'text-slate-300'}`}
                                            >
                                                {c.name}
                                            </div>
                                        ))}
                        ...
                        {/* Date Picker */}
                        <div className="flex-1 relative">
                            <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none z-10" size={16} />
                            <input
                                type="date"
                                value={selectedDate}
                                onChange={(e) => setSelectedDate(e.target.value)}
                                className="w-full bg-slate-800 text-white pl-9 pr-4 py-2.5 rounded-lg border border-white/10 text-sm outline-none focus:border-blue-500 shadow-sm appearance-none relative z-0"
                            />
                        </div>
                    </div>

                    {/* Segments List */}
                    <div className="flex-1 overflow-y-auto p-4 bg-transparent z-0">
